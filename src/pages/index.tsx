...
export default function Search({ makes, models, singleColumn }: SearchProps) {
  ...
  const newMakes = [...new Set(makes.map((el) => el.make))].map((make) =>
    makes.find((el) => el.make === make),
  );
  ...
  return (
    <Formik
      ...
      onSubmit={(values) => {
        // ketika search button di klik, maka akan mengupdate URL query params TANPA MELAKUKAN getServerSideProps lagi
        router.push(
          {
            pathname: '/cars',
            query: { ...values, page: 1 },
